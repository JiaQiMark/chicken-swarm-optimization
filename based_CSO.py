...
from fitness_function import search_range_num
...
class Chicken:
    ...
    def __init__(self):
        ...
        self.next_position = np.random.uniform(-search_range_num, search_range_num, Dn)
    ...
    def evaluate(self):
        self.original_position = self.next_position
        self.fitness = fitness_function(self.original_position)
    ...
    def update_location_rooster(self, number_of_groups, rooster):  # Integer , Class Rooster
        ...
        for index in range(0, Dn):
            ...
            self.next_position[index] = np.clip(self.next_position[index], -search_range_num, search_range_num)

    def update_location_hen(self, number_of_groups, rooster):  # Integer , Class Rooster
        '''
            The Rooster being Passed as a Parameter is to locate the Group to which the particular Rooster Belongs To ,
            Including The Various Other Position Avaiable
        '''
        ...
        for index in range(0, number_of_groups):
            # Running a For Loop , Since The Number of Groups are meant to be less than
            # the original Population Created
            # Check if the Rooster of that Group Matches , Our Hens Group or Not
            # Since According to Segragation of The Population in Group , It is meant
            # that eqaul Population will be shared
...
def implementing_cso(population, individuals_group, maximum_generation, self_update_time, FL=0.5):
    ...
    while iteration_test_cases < maximum_generation:
        ...
        if iteration_test_cases % self_update_time == 0:
            ...
            bundary_num_hen_chicks = int(((population_size - number_of_groups) / 2) + number_of_groups)
        ...
        # It Starts Here!!!!
        '''
            Once All The Roosters, Chickens and Hens are Initalized , 
            We need to Update The Location for Every Fall Iteration Allowed in the Loop
        '''
...
from fitness_function import Dn, population_size, group_size, max_iterations
